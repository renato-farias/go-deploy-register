...
func DBConnect() {
    LoadConfig()
    param := fmt.Sprintf("%s:%s@tcp(%s:%d)/%s", config.Database.User,
                         config.Database.Pass,
                         config.Database.Host,
                         config.Database.Port,
                         config.Database.Base)

    db, err := sql.Open(config.Database.Type, param)
    dbconn = db
...
func CreateApplications() {
    ...
    name := "applications"
    ansi := "CREATE TABLE IF NOT EXISTS `" + config.Database.Base + "`.`" + name + "` (" +
...
func CreateDeployments() {
    ...
    ansi := "CREATE TABLE IF NOT EXISTS `" + config.Database.Base + "`.`" + name + "` (" +
            ...
            "   REFERENCES `" + config.Database.Base + "`.`applications` (`id`)" +
            "   ON DELETE NO ACTION" +
            "   ON UPDATE NO ACTION)" +
            "ENGINE = InnoDB"
    CreateTable(name, ansi)
